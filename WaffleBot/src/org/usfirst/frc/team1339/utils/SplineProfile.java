...
public class SplineProfile {
	...
	public void calculate(double leftDistance, double rightDistance){
		...
		if(getState() == MotionState.SPLINE){
			splineCalculate(dt, leftDistance, rightDistance);
			return;
		}
	...
	private void splineCalculate(double delta_t, double leftDistance, double rightDistance){
		double dt = delta_t;
		...
		/*
		System.out.println(outerCircDistance + " distance");
		System.out.println(splineAngle + " anlge");
		System.out.println(splineRadius + " radius");*/
				
		nextOuterSegment.pos = splineCruiseVel * dt;
		nextOuterSegment.vel = splineCruiseVel;
		nextOuterSegment.acc = 0;
		
		//System.out.println(nextOuterSegment.pos);
		...
		double outsideOutput = Kv * currentOuterSegment.vel + Ka * currentOuterSegment.acc;
		double insideOutput = Kv * currentInnerSegment.vel + Ka * currentInnerSegment.acc;
	
		if (!angleInverted && !splineBackwards) {
			rightOutput = outsideOutput;
			leftOutput = insideOutput;
		}
		else if (!angleInverted && splineBackwards) {
			rightOutput = insideOutput;
			leftOutput = outsideOutput;
		}
		else if (angleInverted && !splineBackwards) {
			rightOutput = insideOutput;
			leftOutput = outsideOutput;
	...
	public boolean isFinishedTrajectory() {
        return Math.abs(currentOuterSegment.pos - totalDistance) < 100;
